...
| 2026-07-30 | Docker application builds | 12 | proved runtime layers, removed a vulnerable unused binary, and produced final evidence image | rebuild only when source/runtime inputs change; never rebuild for docs |
| 2026-07-30 | Broker smoke/baseline generations | 6 | resolved health, tmpfs, startup, and evidence issues; retained one clean baseline | use fail-fast startup, one smoke during development, one baseline after a clean commit |
...
| 2026-07-30 | Host-Java shortcut failure | 2 | confirmed that `-SkipDocker` is not valid on this host | use the Docker-backed validator directly when Java is absent |
| 2026-07-30 | Manual GitHub Actions dispatches | 4 | three runs isolated one new platform defect each; fourth run completed green | use PR-triggered CI after establishing a conflict-free base; do not rerun unchanged failures |
| 2026-07-30 | Cross-platform CI defects | 3 | fixed PowerShell constant collision, Unix dotfile metadata, and UID 10001 bind-file permission | exercise shared scripts under pwsh/Linux and pre-create writable bind files before portfolio rollout |
| 2026-07-30 | GitHub connector transport/mutation errors | 2 | retry confirmed one transient fetch and one partial PR-body update | inspect authoritative remote state before retrying a mutation |
